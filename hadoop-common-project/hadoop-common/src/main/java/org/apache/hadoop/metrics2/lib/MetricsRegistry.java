...
public class MetricsRegistry {
   ...
   * @param valueName   of the metric (e.g., "Time" or "Latency")
   * @param extended    produce extended stat (stdev, min/max etc.) if true.
   * @param numBytes reference to total processed number of bytes
   * @return a new mutable stat metric object
   */
  public synchronized MutableStatShare newStatShare(String name, String desc,
                                          String sampleName, String valueName, boolean extended, MutableCounterLong numBytes) {
    checkMetricName(name);
    MutableStatShare ret =
      new MutableStatShare(name, desc, sampleName, valueName, extended, numBytes);
    metricsMap.put(name, ret);
    return ret;
  }
  ...
  public synchronized void add(String name, long value) {
    ...
    if (m != null) {
      ...
      else if (m instanceof MutableStatShare) {
        ((MutableStatShare) m).add(value);
      }
      else {
...
//    for (String classId : weightsMap.keySet()) {
//      builder.tag(weightsMap.get(classId).info(), weightsMap.get(classId).toString());
//    }
  }
...
//      weightsMap.put(name, new MetricsList(new MetricsInfoImpl(name, "list of values")));
//    }
//    weightsMap.get(name).incr();
//  }
}
