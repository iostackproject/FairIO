 ...
 *   4 longs:
 *   +0 one for the blockId
 *   +1 one for the block length
 *   +2 one for the generation stamp
 *   +3 and one for the classId;
 * - followed by the invalid replica represented with 4 -1s;
 * - followed by the under-construction replica list (the invalid replica)
 *   where each replica is represented by 5 longs:
 *   +0 one for the blockId
 *   +1 one for the block length
 *   +2 one for the generation stamp
 *   +3 one for the classId      // TODO FIXED
 *   +4 and one for the replica state;
 */
@InterfaceAudience.Private
@InterfaceStability.Evolving
public class BlockListAsLongs implements Iterable<Block> {
  /**
   * A finalized block as 3+1 longs
   *   block-id and block length and generation stamp and class-id
   */
  private static final int LONGS_PER_FINALIZED_BLOCK = 4;

  /**
   * An under-construction block as 4+1 longs
   *   block-id and block length, generation stamp and replica state and class-id
   */
  private static final int LONGS_PER_UC_BLOCK = 5;
  ...
  public class BlockReportIterator implements Iterator<Block> {
    ...
    public Block next() {
      block.set(blockId(currentBlockIndex),
                ...
                blockClassId(currentBlockIndex)
                );
   ...
   * Returns the generation stamp of the specified replica of the block report.
   */
  private long blockClassId(int index) {
  ...
  private ReplicaState blockReplicaState(int index) {
    ...
    return ReplicaState.getState((int)blockList[index2BlockId(index) + 4]);
  }
