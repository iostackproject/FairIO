...
public class ReplicaInPipeline extends ReplicaInfo
   ...
   */
  public ReplicaInPipeline(long blockId, long genStamp, long classId,
        FsVolumeSpi vol, File dir) {
    this( blockId, 0L, genStamp, classId, vol, dir, Thread.currentThread());        // TODO FIXED
  }

  @VisibleForTesting
  public ReplicaInPipeline(long blockId, long genStamp,
                           FsVolumeSpi vol, File dir) {
    this( blockId, 0L, genStamp, 0, vol, dir, Thread.currentThread());        // TODO FIXED
  }
  ...
  ReplicaInPipeline(Block block,
    ...
    this( block.getBlockId(), block.getNumBytes(), block.getGenerationStamp(), block.getClassId(),
        vol, dir, writer);
  }
   ...
   * @param writer a thread that is writing to this replica
   */
  ReplicaInPipeline(long blockId, long len, long genStamp, long classId,
      FsVolumeSpi vol, File dir, Thread writer ) {
    super( blockId, len, genStamp, classId, vol, dir);       // TODO FIXED
    this.bytesAcked = len;
