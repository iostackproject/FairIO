...
public class Block implements Writable, Comparable<Block> {
  ...
  public static final Pattern metaFilePattern = Pattern
      .compile(BLOCK_FILE_PREFIX + "(-??\\d++)_(\\d++)_(\\d++)\\" + METADATA_EXTENSION
          + "$");
  ...
  public static long filename2id(String name) {
    ...
    public static long filename2classId(String name) {
        Matcher m = blockFilePattern.matcher(name);
  ...
  public static long getGenerationStamp(String metaFile) {
    ...
    return m.matches() ? Long.parseLong(m.group(3))
  ...
  private long classId;     // TODO FIXED classid en block
  ...
  public Block(final long blkid, final long len, final long generationStamp) {
    set(blkid, len, generationStamp, 0);
  }

  public Block(final long blkid, final long len, final long generationStamp, final long classId) {
    set(blkid, len, generationStamp, classId);
  }
  ...
  public Block(Block blk) {
    this(blk.blockId, blk.numBytes, blk.generationStamp, blk.getClassId());
  }

  /**
   * Find the blockid from the given filename
   */
  public Block(File f, long len, long genstamp) {       // no es fa servir?!
    this(filename2id(f.getName()), len, genstamp, 0);
  }

  public void set(long blkid, long len, long genStamp, long classId) {
    this.blockId = blkid;
    this.numBytes = len;
  ...
  */
  ...
  public String getBlockName() {
    return BLOCK_FILE_PREFIX + String.valueOf(blockId) + "_" + String.valueOf(classId);     // TODO TODO add classid
  }
