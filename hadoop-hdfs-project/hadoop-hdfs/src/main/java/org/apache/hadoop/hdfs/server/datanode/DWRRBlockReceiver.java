 ...
 */
class DWRRBlockReceiver implements Closeable {
  ...
  private LinkedList<DWRRWriteRequest> toBeWritten;

  DWRRBlockReceiver(final ExtendedBlock block, final DataInputStream in,
    ...
    try {
      this.toBeWritten = new LinkedList<DWRRWriteRequest>();
      this.block = block;
      this.in = in;
      this.inAddr = inAddr;
      this.myAddr = myAddr;
      ...
      DataNode.LOG.warn("IOException in DWRRBlockReceiver constructor. Cause is ",
        cause);
  ...
  private int receivePacket() throws IOException {
    ...
    if (mirrorOut != null && !mirrorError) {
      try {
        ...
        long duration = Time.monotonicNow() - begin;
        if (duration > datanodeSlowLogThresholdMs) {
          LOG.warn("Slow DWRRBlockReceiver write packet to mirror took " + duration
...
//          }
          ...
          toBeWritten.add(new DWRRWriteRequest(dataBuf.array().clone(), startByteToDisk, numBytesToDisk, checksumBuf.duplicate(), checksumBuf.array().clone(), checksumLen, len, offsetInBlock, syncBlock, lastPacketInBlock));
  ...
  private void finalizeReceivePacket(DWRRWriteRequest req) throws IOException {
    ...
    try {
      ...
      long duration = Time.monotonicNow() - begin;
      if (duration > datanodeSlowLogThresholdMs) {
        LOG.warn("Slow DWRRBlockReceiver write data to disk cost:" + duration
          + "ms (threshold=" + datanodeSlowLogThresholdMs + "ms)");
      }
  ...
  class PacketResponder implements Runnable, Closeable {
      ...
      while (isRunning() && !lastPacketInBlock) {
        ...
        try {
          ...
          */
          ...
          LOG.warn("IOException in DWRRBlockReceiver.run(): ", e);
          if (running) {
            datanode.checkDiskError();
            LOG.info(myString, e);
            running = false;
     ...
     * Finalize the block and close the block file
     *
     * @param startTime time when DWRRBlockReceiver started receiving the block
     */
    private void finalizeBlock(long startTime) throws IOException {
      DWRRBlockReceiver.this.close();
      final long endTime = ClientTraceLog.isInfoEnabled() ? System.nanoTime()
        : 0;
      block.setNumBytes(replicaInfo.getNumBytes());
      datanode.data.finalizeBlock(block);
